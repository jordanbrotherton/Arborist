...
def set_default(checksum: str):
    """Elevated OSTree helper function.
       Sets a supplied deployment as default.

    Args:
        checksum (str): The checksum of the deployment to set as default.

    Raises:
        Exception: If the deployment is nonexistent or already default.
    """
    ...
    new_deployments = [current_deployment]
    for deployment in deployments:
        if deployment.get_csum() != checksum:
